...
1. For `.pptx` and `.csv` files, use the explicit `markitdown` MCP tool if it is available.
2. Convert the file to markdown first. Do not work from the binary file directly.
3. Summarize the converted content into the smallest useful form for the task:
   - deck outline
   - speaker-note bullets
   - table summary
   - key metrics or themes
   - open questions or gaps
4. Carry forward only the relevant excerpts into later prompts or artifacts.
5. Prefer filenames, slide numbers, section headings, or row-group labels when citing source material.
